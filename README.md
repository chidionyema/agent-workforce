...
Decision record: `crew/docs/plans/2026-09-01-brand-new-infra-crew-on-crewai.md` (crew#729).
...
| Identity | an installation token for the `infra-crew` lane of the estate's one GitHub App: read, write on `infra-crew/*` branches, open pull requests, comment, read Actions logs |
| Laws | crewAI Knowledge over the law and standards files, embedded through the router |
| Memory | crewAI long-term memory on the workload's volume (`INFRA_CREW_STORAGE_DIR`) |
...
`src/infra_crew/estate.py` and starts it with one argument, the board issue number:
