...
| Seeded issue | the workload runs `infra-crew <number>` against a `lane:infra` ticket | a comment ending `Optimised:` and a pull request from `infra-crew/<number>` exist on the board |
| Trace proved from the backend | the last line of the run log | `langfuse traces for session infra-crew-<number>-<id>: N` with N > 0; exit 0 |
...
- Identity is a lane on the estate's one GitHub App, not a second App (one platform, one
  identity layer; see the `idp` runbook `docs/runbooks/infra-crew-identity.md`).
